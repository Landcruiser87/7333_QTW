...
estXYkmin = predXY1(newSignals = onlineSummary[ , 6:11], 
...
Looks like the best value for K was about `r kMin`, with an error of `r rmseMin`. We can use cross validated value of K to make a prediction on the online data, which has an RMSE of `r calcError(estXYkmin, actualXY)`. We will see how this stacks up compared to the others. Let us now turn our attention to router B:
...
Looks like the best value for K was about `r kMin`, with an error of `r rmseMin`. We can use cross validated value of K to make a prediction on the online data, which has an RMSE of `r calcError(estXYkmin, actualXY)`. Lets compare the results to those of router B:
...
```{r}
data.frame(router = c("router a","router b"), `minimum rmse` = c(min(errdf$router_a), min(errdf$router_b)))
```
...
estXYkmin = predXY2(newSignals = onlineSummary[ , 6:12], 
...
Looks like the best value for K was about `r kMin`, with an error of `r rmseMin`. We can use cross validated value of K to make a prediction on the online data, which has an RMSE of `r calcError(estXYkmin, actualXY)`. Lets compare to our previous results:
...
```{r}
data.frame(router = c("router a","router b", "both"), `minimum rmse` = c(min(errdf$router_a), min(errdf$router_b), min(errdf$both)))
```
...
ok <- estXYkmin
estXYkmin = predXY1(newSignals = onlineSummary[ , 6:11], newAngles = onlineSummary[ , 4], offlineSummary, numAngles = 1, k = kMin)
```
