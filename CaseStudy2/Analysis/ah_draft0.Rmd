...
Here we plot the routes of the trackers
...
estXYkmin = predXY(newSignals = onlineSummary[ , 6:11], 
                 newAngles = onlineSummary[ , 4], 
...
## Dropping Mac 00:0f:a3:39:e1:c0
...
offlineSummary = subset(orig_offlineSummary, mac != subMacs[1])
```
...
#Now we drop the second MAC 00:0f:a3:39:e1:c0
offline <- readData()
offline$posXY <- paste(offline$posX, offline$posY, sep = "-")
...
estXYk5 = predXY(newSignals = onlineSummary[ , 6:11], 
                 newAngles = onlineSummary[ , 4], 
                 offlineSummary, numAngles = 1, k = 6)
...
# new reshape function for 7 column data for cross val
reshapeSS2 <- function(data, varSignal = "signal", keepVars = c("posXY", "posX", "posY", "orientation", "angle")) {
  byLocation <- with(data, by(data, list(posXY),
                              function(x) {
                                ans = x[1, keepVars]
                                avgSS = tapply(x[ , varSignal ], x$mac, mean)
                                y = matrix(avgSS, nrow = 1, ncol = 7, # 7 columns = 7 macs
                                           dimnames = list(ans$posXY,
                                                           names(avgSS)))
                                cbind(ans, y)
                              }))
...
length(tapply(1:nrow(offline), offline$mac, mean)) 
reshapeSS3 = function(data, varSignal = "signal", keepVars = c("posXY", "posX", "posY")) {
  byLocation = with(data, by(data, list(posXY),
                             function(x) {
                               ans = x[1, keepVars]
                               avgSS = tapply(x[ [varSignal] ], x$mac, mean)
...
# new permuteLocs
# 12 fold cv
permuteLocs2 <- sample(unique(offline$posXY))
permuteLocs2 <- matrix(permuteLocs2, ncol = 2, nrow = floor(length(permuteLocs2)/2))

selectTrain2 <-  function(angleNewObs, signals, m) {
 refs = seq(0, by = 45, length = 8)
  nearestAngle = roundOrientation(angleNewObs)
  if (m %% 2 == 1) {
    angles = seq(-45 * (m - 1) / 2, 45 * (m - 1) / 2, length = m)
  ...
  angles = sort(angles)
  offlineSubset = signals[signals$angle %in% angles,]
  trainSS = reshapeSS3(offlineSubset, varSignal = "avgSignal")
 return(trainSS)
}
...
keepVars = c("posXY", "posX","posY", "orientation", "angle")
onlineCVSummary = reshapeSS3(offline, keepVars = keepVars)
onlineFold = subset(onlineCVSummary, 
                    posXY %in% permuteLocs2[ , 1])
offlineFold = subset(offlineSummary,
                     posXY %in% permuteLocs2[ , -1])
length(onlineFold)
str(offlineFold)
estFold = predXY2(newSignals = onlineFold[ , 6:12], newAngles = onlineFold[ , 5], offlineFold, numAngles = 1, k = 3)
