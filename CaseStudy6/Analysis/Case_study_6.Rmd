...
pander::pander(
 list( Accuracy = "Accuracy is defined as the number of correct predictions divided by the total number of predictions.",
       ...
       F1-Score = "F1 score is defined as a weighted average of Precision and Recall. This metric is more useful when you have uneven class balances but it sometimes useful as it includes false postivies and false negatives. ",
            list("2*(Recall*Precision)/(Recall + Position")
...
treedepth <- function(fit) {
  nodes <- as.numeric(rownames(fit$frame))
  max(rpart:::tree.depth(nodes))
}
#Possibly sample these because its 36000 combinations
workers <- makeCluster(parallel::detectCores()-1)
registerDoParallel(workers)
invisible(clusterCall(workers, function() library(rpart)))
res <- foreach(i =  1:nrow(params), .combine=rbind) %dopar% {
  model <- rpart(isSpam ~ ., data = spam_train, method="class",
                control = do.call(rpart.control,list(params[i,]))
  )
  preds <- predict(model, newdata=spam_test, type="class")
  confusion_mat <- function(preds, truth) {
    return(table(preds,truth))
  }
  accuracy <- function(preds, truth=spam_test$isSpam) {
  ...
  metr <- get_metrics(preds)
  ntree <- c("treedepth" = treedepth(model))
  return(c(params[i,],metr, ntree))
}
save(data.frame(res), "grid.Rda")
